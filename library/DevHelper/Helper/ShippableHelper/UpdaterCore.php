...
/**
 * Class DevHelper_Helper_ShippableHelper_UpdaterCore
 * @version 1
 */
class DevHelper_Helper_ShippableHelper_UpdaterCore
{
    ...
    {
        if (isset($GLOBALS[DevHelper_Helper_ShippableHelper_Updater::KEY]['setupMethod'][$config['apiUrl']])) {
            if (XenForo_Application::debugMode()) {
                die(sprintf('Updater for %s has been setup by %s already!', $config['apiUrl'],
                    $GLOBALS[DevHelper_Helper_ShippableHelper_Updater::KEY]['setupMethod'][$config['apiUrl']]));
            }
            return;
        }
        $GLOBALS[DevHelper_Helper_ShippableHelper_Updater::KEY]['setupMethod'][$config['apiUrl']] = __METHOD__;
